...
st.title("Аналіз стійкості лесового схилу")
st.caption("Метод Бішопа | Двошарова геологія | Природні армуючі матеріали")
...
with st.sidebar:
    ...
    with st.expander("Шар 1 — Лес (верхній)", expanded=True):
        st.caption("Верхній шар ґрунту")
        depth1 = st.slider(
            "Глибина шару 1 (м від поверхні)",
            1.0, H, min(H/2, 5.0), 0.5
        )
        st.markdown("**Природний стан:**")
    ...
    with st.expander("Зовнішнє навантаження", expanded=False):
...
try:
    with st.spinner("Розраховуємо..."):
        ...
        # Сценарій 1 — природний двошаровий
        s1 = build_slope(
            c1_nat, phi1_nat, g1_nat
        )
        fs1 = s1.get_min_FOS()

        # Сценарій 2 — замочений двошаровий
        s2 = build_slope(
...
# ===== СХЕМА ГЕОЛОГІЇ =====
    st.subheader("Геологічний розріз")
    ...
    # Шар 2 — суглинок (повний з обома платформами)
    fig_geo.add_trace(go.Scatter(
        ...
        fill="toself",
        fillcolor="rgba(72, 209, 204, 0.6)",
        line=dict(color="rgba(0,150,150,0.8)", width=1.5),
        name=f"Шар 2 — {layer2_type}",
        mode="lines"
    ))

    # Шар 1 — лес (трикутник + ліва платформа)
    fig_geo.add_trace(go.Scatter(
        ...
        line=dict(color="rgba(200,80,60,0.8)", width=1.5),
        name="Шар 1 — Лес",
        mode="lines"
    ))

    # Чорна лінія схилу з платформами
    fig_geo.add_trace(go.Scatter(
    ...
    fig_geo.add_annotation(
        x=-platform * 0.5,
        y=(layer1_bottom + H) / 2,
        text=f"Шар 1: Лес<br>c={c1_nat} кПа | φ={phi1_nat}°",
        showarrow=False,
        font=dict(size=11, color="rgb(150,50,20)"),
    ...
    fig_geo.add_annotation(
        x=L * 0.55,
        y=(layer1_bottom + bottom) / 2,
        text=f"Шар 2: {layer2_type}<br>c={c2_layer} кПа | φ={phi2_layer}°",
        showarrow=False,
        font=dict(size=11, color="rgb(0,100,100)"),
        bgcolor="rgba(255,255,255,0.8)",
    ...
    fig_geo.update_layout(
        ...
        yaxis=dict(
            title="Позначка (м)",
            range=[bottom - 0.5, H + 1]
        ),
        margin=dict(t=10, b=20, l=50, r=160),
        plot_bgcolor="rgba(235,245,255,0.6)"
    )
    ...
    m1.metric("Fs природний", f"{fs1:.3f}",
              fs_label(fs1), delta_color=fs_color(fs1))
    m2.metric("Fs замочений", f"{fs2:.3f}",
              fs_label(fs2), delta_color=fs_color(fs2))
    m3.metric(f"Fs + армування", f"{fs3:.3f}",
              fs_label(fs3), delta_color=fs_color(fs3))
    m4.metric("Втрата при замочуванні",
              f"-{fs1-fs2:.3f}",
              f"-{(fs1-fs2)/fs1*100:.1f}%",
              delta_color="inverse")
    m5.metric("Ефект армування",
              f"+{fs3-fs2:.3f}",
              f"+{(fs3-fs2)/max(fs2,0.001)*100:.1f}%",
              delta_color="normal")
    ...
    with tab1:
        ...
        if mat_fs:
            ...
            fig_mat.add_hline(y=1.5, line_dash="dash",
                              line_color="green",
                              annotation_text="Норма Fs = 1.5")
            fig_mat.add_hline(y=1.0, line_dash="dash",
                              line_color="red",
                              annotation_text="Зсув Fs = 1.0")
    ...
    with tab2:
        ...
        with col_a:
            ...
            fig_bar.add_hline(y=1.5, line_dash="dash",
                              line_color="green",
                              annotation_text="Fs = 1.5 (ДБН)")
            fig_bar.add_hline(y=1.0, line_dash="dash",
                              line_color="red",
                              annotation_text="Fs = 1.0 (зсув)")
        ...
        with col_b:
            ...
            if fs3 >= 1.5:
                st.success(f"{arm_type} забезпечує Fs = {fs3:.3f} — норма!")
            elif fs3 >= 1.2:
                st.warning(f"Fs = {fs3:.3f} — покращення є, але норма 1.5 не досягнута")
            else:
                st.error(f"Fs = {fs3:.3f} — потрібні додаткові заходи")
            need = max(0, 1.5 - fs3)
            st.metric("Потрібно ще до Fs 1.5", f"+{need:.3f}")
    ...
    with tab3:
        ...
        fig_line.add_trace(go.Scatter(
            x=x_vals, y=r1, name="Природний (2 шари)",
            line=dict(color="#2196F3", width=2.5), mode="lines+markers"))
        fig_line.add_trace(go.Scatter(
            x=x_vals, y=r2, name="Замочений (2 шари)",
            line=dict(color="#f44336", width=2.5), mode="lines+markers"))
        fig_line.add_trace(go.Scatter(
            x=x_vals, y=r3, name=f"З армуванням ({arm_type[:12]})",
            line=dict(color="#4caf50", width=2.5), mode="lines+markers"))
        fig_line.add_hline(y=1.5, line_dash="dash",
                           line_color="green", annotation_text="Fs=1.5")
        fig_line.add_hline(y=1.0, line_dash="dash",
                           line_color="red", annotation_text="Fs=1.0")
        if x_cur:
            fig_line.add_vline(x=x_cur, line_dash="dot",
                               line_color="gray", annotation_text="Поточне")
        fig_line.update_layout(
            xaxis_title=x_label, yaxis_title="Fs",
    ...
    with tab4:
        ...
        try:
            _s = s2 if sc4 == "Замочений" else s3
